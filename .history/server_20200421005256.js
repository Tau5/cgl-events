...
class User {
	...
	levelup() {
		...
		GUILD.channels.cache.get(this.lastseen).send(
			`¡${this.get_guild_user().displayName} has subido a nivel ${this.level}!`
		);
		this.calculate_levelup_req();
		if (RANKS[""+this.level] != undefined) {
			let rank_name = teams[this.team].rank_names[RANKS[""+this.level]];
			GUILD.channels.cache.get(this.lastseen).send(
				`¡${this.get_guild_user().displayName} has llegado al rango ${rank_name} 🎉!`
			);
