...
*/
class User {
	constructor(identifier, userid, team, points, level, levelup_req, rank, tokens, lastseen) {
		if (!teams[team]) {
...
function db_save() {
	...
	fs.writeFile("./users_db.json", JSON.stringify(teams_db_temp), () => {
		console.log("[db_save] Team DB saved!")
	});
}
function load_teams() {
	...
	if (fs.existsSync("./teams_db.json")) {
		...
		for (i in teamdb) {
			team_info = teamdb[i]
			new_team = new Team(team_info.name, team_info.points, team_info.rank_names);
			teams.push(new_team)
		}
